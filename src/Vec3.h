...
public:
    ...
    __device__ static Vec3 randomUnitVector(curandState* rand);
};
...
__device__ inline Vec3 operator*(const Vec3& a, const double t) {
    return {a.x()*t, a.y()*t, a.z()*t};
}

__device__ inline Vec3 operator*(const double t, const Vec3& a) {
    return a*t;
}
...
__device__ inline static Vec3 cross(const Vec3& a, const Vec3& b) {
    return {
        a.y()*b.z()-a.z()*b.y(),
        a.z()*b.x()-a.x()*b.z(),
        a.x()*b.y()-a.y()*b.x(),
...
__device__ inline static Vec3 refract(const Vec3& v, const Vec3& n, double refractiveIndexRatio) {
